...
import withHistoryCache from "./node_modules/react-storefront/router/withHistoryCache";
import { useObserver } from "mobx-react";
import Skeleton from "./node_modules/react-storefront/Skeleton";
import useTraceUpdate from "../../pages/p/node_modules/react-storefront/hooks/useTraceUpdate";
  ...
  return useObserver(() => {
    ...
    const fetchMore = useCallback(async () => {
      ...
      const more = await fetch(
        `/api/s/${subcategory.id}?page=${store.page + 1}`
      ).then(res => res.json());
